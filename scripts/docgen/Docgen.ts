...
import { resolvePackageEntrySourcePaths } from './resolvePackageEntrySources';
...
export class Docgen {
  ...
  private async generateDoc(packageName: string) {
    const packageSources = resolvePackageEntrySourcePaths(this.packagesRootPath, packageName);
    ...
    for (const packageSrc of packageSources) {
      for (const docData of parse(packageSrc, parserOptions)) {
        if (seenDisplayNames.has(docData.displayName)) {
          continue;
        }
        seenDisplayNames.add(docData.displayName);
