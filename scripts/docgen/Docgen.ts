...
export class Docgen {
  ...
  readonly #docPlaceholder: [string, string];
  ...
  #readReadmeFile(packageName: string) {
    try {
  ...
  #writeDocsSectionToReadmeFile(packageName: string, doc: string) {
  ...
  #generateDoc(packageName: string) {
    ...
    return parse(packageSrc, this.#parserOptions).map(docData => {
      ...
      console.log(`✅ doc generated for ${packageName}/README.md - ${docData.displayName}`);
      return doc;
    });
  }
