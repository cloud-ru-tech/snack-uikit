...
export const getCalendarTextSnapshot = async (page: Page) =>
  await page.evaluate(() => {
...
export const focusItem = async (page: Page, index: number) => {
...
export const getPresetsItemsLabels = async (page: Page) =>
  await page.evaluate(() => {
    ...
    for (const element of elements) {
      itemsLabels.push(element.textContent || '');
    }
