...
test.page(getPage())('Should set checked for inner checkbox by click', async t => {
  await t.click(Selector(BUTTON_SELECTOR));
  await t.expect(Selector(ITEM_SELECTOR).nth(0).find('input[type=checkbox]').checked).ok();
  await t.expect(Selector(ITEM_SELECTOR).nth(1).find('input[type=checkbox]').checked).notOk();
