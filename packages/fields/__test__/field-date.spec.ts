...
test.describe('Field Date', () => {
  ...
  const waitForCalendar = (getByTestId: (testId: string) => Locator) =>
    expect(getByTestId(CALENDAR_TEST_ID)).toBeVisible({ timeout: 10000 });
  ...
  test.describe('calendar selection', () => {
    ...
    test("[mode='date'] Should select value from calendar with mouse", async ({ gotoStory, getByTestId }) => {
      ...
      await input.click();
      await waitForCalendar(getByTestId);
    ...
    // select data
    test("[mode='date-time'] Should select value from calendar with mouse", async ({
      gotoStory,
      getByTestId,
      page,
    }) => {
